...
/**
 * POST /api/gastos-reparables/ocr
 * Procesa una imagen de planilla de gastos reparables con OCR (Gemini Vision)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📄 [GASTO REPARABLE OCR] Procesando imagen:', file.name, `(${(file.size / 1024).toFixed(0)}KB)`)
    ...
    const uploadDir = join(process.cwd(), 'public', 'uploads', session.user.organizationId, 'gastos-reparables')
    if (!existsSync(uploadDir)) {
      await mkdir(uploadDir, { recursive: true })
    }

    const timestamp = Date.now()
    const fileName = `${timestamp}-gasto-reparable.jpg`
    const filePath = join(uploadDir, fileName)
    await writeFile(filePath, processedBuffer)
    const imageUrl = `/api/uploads/${session.user.organizationId}/gastos-reparables/${fileName}`
    ...
    const extractedData = await geminiService.analyzePlanillaGastoReparable(processedBuffer)
    ...
    console.error('❌ Gasto Reparable OCR error:', error)
    return NextResponse.json(
