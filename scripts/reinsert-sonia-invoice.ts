...
async function main() {
  ...
  if (ocrData?.rawData?.items && Array.isArray(ocrData.rawData.items) && ocrData.rawData.items.length > 0) {
    ...
    items.forEach((item, idx) => {
      console.log(`   Item ${idx + 1}: ${item.descripcion?.substring(0, 50)}`)
      if (item.descripcion && item.descripcion.length > 500) {
        console.log(`      ⚠️  Descripción truncada de ${item.descripcion.length} a 500 caracteres`)
      }
      if (item.codigoProducto && item.codigoProducto.length > 100) {
        console.log(`      ⚠️  Código truncado de ${item.codigoProducto.length} a 100 caracteres`)
      }
    })
  } else {
  ...
  try {
    ...
    console.log('\n🎉 ¡ÉXITO! La factura de Sonia Monsalve ahora está en SQL Server')
  } catch (error: any) {
